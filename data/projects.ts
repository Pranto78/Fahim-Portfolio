...
export const PROJECTS: Project[] = [
  {
    ...
    auth: "JWT (access 1d / refresh 30d) with Passport (local + Google OAuth20) on web; react-native-keychain for secure token storage on mobile with silent logout on token expiry.",
    ...
    role: "Worked across the mobile app and backend — merchandising execution screens (gift status, stock carton/pcs, POSM/PMM), role-based access, and Android Gradle/AGP build & CI fixes.",
  },
  {
    id: "nupath",
    name: "NuPath — NFL",
    tag: "Mobile App for the NFL Alumni",
    blurb: "A React Native app for the NFL Alumni Association.",
    unique:
  ...
  },
  {
    ...
    auth: "JWT (Bearer access + refresh, persisted). Login via POST /auth/app-login with role-based user models (User / Dependent / etc.).",
    ...
    role: "Resolved Android Gradle/AGP build issues, fixed safe-area/navigation overlap, and worked on UI flows and feature screens.",
  },
  {
    id: "omd-portal",
    ...
    role: "Worked on securing backend API endpoints — role-based access control and an orgAccessGuard middleware to prevent cross-organization data access.",
  },
];
